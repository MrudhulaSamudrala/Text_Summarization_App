# Fix for asyncio RuntimeError when used in Streamlit
import asyncio
...
def load_abstractive_models():
    try:
        # Get the path to the root directory (one level up from pages)
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        model_dir = os.path.join(root_dir, 'Models')
        
        # Load tokenizers
        with open(os.path.join(model_dir, 'x_tokenizer.pickle'), 'rb') as handle:
            x_tokenizer = pickle.load(handle)
        with open(os.path.join(model_dir, 'y_tokenizer.pickle'), 'rb') as handle:
            y_tokenizer = pickle.load(handle)

        # Load max lengths
        max_text_len = int(np.load(os.path.join(model_dir, 'max_text_len.npy')))
        max_summary_len = int(np.load(os.path.join(model_dir, 'max_summary_len.npy')))

        # Load models
        encoder_model = load_model(os.path.join(model_dir, 'encoder_model1.h5'), 
                                 custom_objects={'AttentionLayer': AttentionLayer})
        decoder_model = load_model(os.path.join(model_dir, 'decoder_model1.h5'), 
                                 custom_objects={'AttentionLayer': AttentionLayer})
