...
def summarize_text(text, ratio=0.3, model_type="extractive"):
    ...
    if model_type == "extractive":
        ...
        return summarize_text_abstractive(text, abstractive_models)
...
def display_summary_with_audio(summary, source_name="summary"):
    """Display summary with audio controls and download options."""
    st.write(summary)
    ...
    with col1:
        ...
        st.download_button(
            ...
            data=audio_bytes,
            file_name=f"{source_name}_summary.mp3",
            mime="audio/mp3"
        )
    ...
    with col2:
        ...
        txt_file = BytesIO()
        txt_file.write(summary.encode('utf-8'))
        txt_file.seek(0)
    ...
    # Clean up the temporary file after download buttons are created
    try:
...
with tab1:
    ...
    if st.button("Summarize Text", key="text_button"):
        if user_input.strip():
            ...
            display_summary_with_audio(summary, "text_input")
    ...
    if pdf_file is not None:
        ...
        if st.button("Extract and Summarize", key="pdf_button"):
            ...
            if text and text != "No text extracted from PDF.":
                ...
                st.subheader("Summary:")
                display_summary_with_audio(summary, "pdf_summary")
    ...
    if image_file is not None:
        ...
        if st.button("Extract and Summarize", key="image_button"):
            ...
            if text and text != "No text extracted from image.":
                summary = summarize_text(text, ratio=summary_ratio, model_type=model_type.lower())
                st.subheader("Summary:")
                display_summary_with_audio(summary, "image_summary")
        ...
        if extract_option == "Process as CSV (Summarize each row)":
            ...
            if 'Text' in df.columns:
                ...
                for idx, row in df.iterrows():
                    ...
                    display_summary_with_audio(row['Summary'], f"csv_row_{idx}")
            ...
            if st.button("Extract and Summarize", key="csv_extract_button"):
                ...
                if text and text != "No text extracted from CSV." and not text.startswith("Error"):
                    summary = summarize_text(text, ratio=summary_ratio, model_type=model_type.lower())
                    st.subheader("Summary:")
                    display_summary_with_audio(summary, "csv_summary")
